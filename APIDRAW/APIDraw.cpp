...
enum {
	...
	Menu_Delete, Menu_Clear, Menu_Prop,Menu_ImgOption,Menu_TxtOption,
	Menu_Forward, Menu_FarForward, Menu_Backward, Menu_FarBack,Menu_Copy,Menu_Paste,
...
void PrintBitmap(HDC hmemdc,void* image,RECT& rt) {
	BITMAPFILEHEADER* fh;
	BITMAPINFOHEADER* ih;
	BYTE* pbuf;
	fh = (BITMAPFILEHEADER*)image;
...
LRESULT CALLBACK TxtDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
struct ShapeList {
	struct ShapeData {
		...
		UCHAR alpha;
		UCHAR thick;
		UCHAR linetype, brushtype, brushmode;
		ShapeData() {
...
private:
	...
	void DrawTracker(HDC hdc) {
		...
		if (shapenum->type == DrawType_Polygon) {
			for (UINT i = 0; i < shapenum->point_len; i++)
...
public:
	ShapeList* shapelist;
	HWND api_hwnd, dialog_hwnd,dlg2_hwnd,dlg3_hwnd;
	HINSTANCE g_hin;
	...
	void Set_ImgDlg() {
		if (dlg2_hwnd)InvalidateRect(dlg2_hwnd, 0, true);
	}
...
void APIClass::KeyDown(WPARAM wp) {
	...
	case'I':
		if (isctrl)
			SendMessage(api_hwnd, WM_COMMAND, Menu_ImgOption, 0);
		return;
	case'T':
		if (isctrl)
			SendMessage(api_hwnd, WM_COMMAND, Menu_TxtOption, 0);
		return;
	}
	if (!shapenum)return;
...
inline Color SetColor2(BYTE alpha, DWORD color) {
	return Color(alpha,
		GetRValue(color),
		GetGValue(color),
		GetBValue(color)
	...
	for (auto d = shapelist->data; d != shapelist->data + shapelist->len; d++) {
		...
		gpen.SetWidth(d->thick);
		{
			...
			case DrawType_Bitmap: 
				...
				PrintBitmap(hmemdc, d->image, d->rt);
				break;
			case DrawType_Meta:
...
void APIClass::Command(USHORT type, WPARAM wp) {
	...
	case Menu_Backward:
		...
		break;
	case Menu_Prop: {
		if (!IsWindow(dialog_hwnd) && !IsWindow(dlg2_hwnd) && !IsWindow(dlg3_hwnd)) {
			WNDCLASSEXW wc = { 0 };
	...
	case Menu_TxtOption: {
		if (!IsWindow(dialog_hwnd) && !IsWindow(dlg2_hwnd) && !IsWindow(dlg3_hwnd)) {
			if (!shapenum) break;
			if (shapenum->type != DrawType_Text)break;
			
			WNDCLASSEXW wc = { 0 };
			wc.cbSize = sizeof(WNDCLASSEXW);
			wc.lpfnWndProc = (WNDPROC)TxtDialogProc;
			wc.hInstance = g_hin;
			wc.hbrBackground = GetSysColorBrush(COLOR_3DFACE);
			wc.lpszClassName = L"DialogClass3";

			RegisterClassExW(&wc);

			dlg3_hwnd = CreateWindowExW(WS_EX_DLGMODALFRAME,//| WS_EX_TOPMOST,
				L"DialogClass3", L"텍스트 속성",
				WS_VISIBLE | WS_SYSMENU | WS_CAPTION, 100, 100, 400, 300,
				api_hwnd, 0, g_hin, NULL);

			EnableWindow(api->api_hwnd, false);
		}
...
void APIClass::RButtonUp(long x, long y) {
	...
	if (!isDrag) {
		...
		AppendMenu(hpop, MF_STRING, Menu_Copy, L"복사\tCtrl+C");
		AppendMenu(hpop, MF_STRING, Menu_Paste, L"붙여넣기\tCtrl+V");
		...
		AppendMenu(hpop, MF_STRING, Menu_TxtOption, L"텍스트 속성\tCtrl+T");
		AppendMenu(hpop, MF_SEPARATOR, 0, 0);
...
enum {
	TxtDlg_Edit1 = 1, TxtDlg_Btn1, TxtDlg_BtnCancel,TxtDlg_Font,

	ImgDlg_BtnLoad=1,ImgDlg_BtnOk, ImgDlg_BtnCancel,
};
LRESULT CALLBACK ImgDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
	case WM_CREATE:
	{
		long y = 20;
		CreateWindow(L"button", L"불러오기", WS_CHILD | WS_VISIBLE
			, 290, y, 80, 25, hwnd, (HMENU)ImgDlg_BtnLoad, 0, 0);
		y += 30;
		CreateWindow(L"button", L"확인", WS_CHILD | WS_VISIBLE
			, 290, y, 80, 25, hwnd, (HMENU)ImgDlg_BtnOk, 0, 0);
		y += 30;
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case ImgDlg_BtnLoad: 
			if (!api->shapenum)break;
			if (api->shapenum->type == DrawType_Bitmap) {}
			else if (api->shapenum->type == DrawType_Meta) {}
			else break;
			{
			OPENFILENAME ofn = { 0 };
			wchar_t filename[256] = L"";
	...
	case WM_PAINT:
	{
		...
		HBITMAP oldbit;*/
		//oldbit = (HBITMAP)SelectObject(memdc,);
		RECT rt = { 10,10,280,250 };
		Rectangle(hdc, 10, 10, 280, 250);
		if (api->shapenum) {
			if (api->shapenum->type == DrawType_Bitmap)
				PrintBitmap(hdc, api->shapenum->image, rt);
			else if (api->shapenum->type == DrawType_Meta)
				PlayPlaceableMeta(hdc, api->shapenum->image, 
					api->shapenum->point_len, &rt);
				//PrintBitmap(hdc, api->shapenum->image, rt);
	...
	//case WM_KILLFOCUS:InvalidateRect(hwnd,0,true); break;
	}
	return(DefWindowProcW(hwnd, msg, wParam, lParam));
}
...
LRESULT CALLBACK TxtDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
	{
		...
		hedit = CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER |
			WS_VSCROLL | WS_HSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_MULTILINE | ES_AUTOHSCROLL,
			10, y, 350, 150, hwnd, (HMENU)TxtDlg_Edit1, 0, NULL);
		y += 160;
		CreateWindow(L"button", L"확인", WS_CHILD | WS_VISIBLE
			, 270, y, 80, 25, hwnd, (HMENU)TxtDlg_Btn1, 0, 0);
		y += 30;
		CreateWindow(L"button", L"취소", WS_CHILD | WS_VISIBLE
			, 270, y, 80, 25, hwnd, (HMENU)TxtDlg_BtnCancel, 0, 0);
		y += 30;
		CreateWindow(L"button", L"폰트 설정", WS_CHILD | WS_VISIBLE
			, 270, y, 80, 25, hwnd, (HMENU)TxtDlg_Font, 0, 0);
	}
	break;
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		...
		break;
	case WM_CLOSE:DestroyWindow(hwnd); EnableWindow(api->api_hwnd,true);
		break;
	}
	return(DefWindowProcW(hwnd, msg, wParam, lParam));
}
